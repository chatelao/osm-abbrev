...
CREATE or REPLACE FUNCTION osmabbrv_street_abbrev_fr(longname text) RETURNS TEXT AS $$
 ...
 BEGIN
  abbrev=longname;
  -- abbrev=regexp_replace(abbrev,'^1([eè]?r)?e Avenue\M','1ʳᵉ Av.');
  -- abbrev=regexp_replace(abbrev,'^([0-9]*)e Avenue','\1e Av.')
...
CREATE or REPLACE FUNCTION osmabbrv_street_abbrev_en(longname text) RETURNS TEXT AS $$
 ...
 BEGIN
  ...
  -- abbrev=regexp_replace(abbrev,'(?<!^|1([eè]?r)?e |[0-9]e )Avenue\M','Ave.');
